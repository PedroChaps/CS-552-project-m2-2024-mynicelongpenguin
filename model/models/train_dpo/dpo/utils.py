...
def create_datasets(tokenizer, data_args, training_args, apply_chat_template=False):
    ...
    for split in data_args.splits.split(","):
        try:
            if data_args.dataset_is_json:
                dataset = load_dataset("json", data_files=f"{data_args.dataset_name}_{split}.jsonl", split="train")
                dprint(f"{data_args.dataset_name}.jsonl")
                dprint("Loaded dataset from json file: ", f"{data_args.dataset_name}_{split}.jsonl")
            else: 
    ...
    print(f"Size of the train set: {len(train_data) if train_data != None else 'None'}. Size of the validation set: {len(valid_data) if valid_data != None else 'None'}")
...
def create_and_prepare_model(args, data_args, training_args):
    ...
    if args.use_unsloth:
        # Load model
        dprint(f"{args.model_name_or_path = }")
        dprint(f"{data_args.max_seq_length = }")
        dprint(f"{args.use_4bit_quantization = }")
